...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostDist in livingGhostPositionDistributions:
            most_likely_pos.append(tuple(ghostDist.argMax()))
        pos_dist = dict()
        for pos in most_likely_pos:
            pos_dist[pos] = self.distancer.getDistance(pacmanPosition, pos)
        dist = float('inf')
        closestGhost = min(pos_dist, key=pos_dist.get)
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            new_dist = self.distancer.getDistance(successorPosition, closestGhost)
            if (new_dist < dist):
                best_act = action
                dist = new_dist
        return best_act
